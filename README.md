...
| `chunks.jsonl` | Knowledge | 3,284 retrieval chunks of AMPL documentation. The LLM searches this file to ground answers with accurate, citable content. |

The two layers are independent: `ampl-guidelines.md` defines *how* to respond; `chunks.jsonl` provides *what* to say. You can extend or replace one without touching the other.
...
ampl-guidelines.md
  ...
  • Applies code style, solver selection, and formulation rules
     │
     ▼
chunks.jsonl  ──►  LLM searches for relevant documentation chunks
...
3. Make sure `chunks.jsonl` is indexed or included in the context window (add it to the relevant include list if needed).
...
2. Upload `ampl-guidelines.md` and `chunks.jsonl` as **Knowledge** files.
3. Enable **File Search** so the GPT retrieves from `chunks.jsonl` automatically.
...
3. Import `chunks.jsonl` into the tool's document/knowledge store and enable RAG retrieval.
...
1. Index `chunks.jsonl` into your vector store. Use the `text` field as the embedding input; store all other fields as metadata.
...
Answer quality depends on the LLM, embedding model, context window, and retrieval settings. `chunks.jsonl` provides the raw knowledge; how well the model synthesises and writes correct AMPL code depends on the model tier. Higher-capability models produce better results.
